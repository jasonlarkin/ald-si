...
struct SYMMETRY{
   double  *rot, *rot2;                                 //rot, rmap is for origin atom
   int *rmap, *r2map;                                      //rot2, r2map are for (0.25,0.25,0.25) atom
   double *order2, *order3, *order4;
   int *iorder2, *iorder3, *iorder4;
...
void sub_realSymmetry(LATTICE *lattice, SYMMETRY *symmetry);
void sub_irrdisp(LATTICE *lattice, SYMMETRY *symmetry, DYNA *dynamic);
//===============================================================
...
int main(){
  ...
  sub_realSymmetry(lattice,symmetry);

  end = clock();
  cpuTime= (end-start)/ (double) (CLOCKS_PER_SEC);
  start = clock();
  cout<<"Symmetries in the real structure identified in "<<cpuTime<<" seconds."<<endl;
  //=================================================================================================

  //======GENERATING FULL FORCE CONSTANT LIST FROM IRREDUCIBLE LIST=====================
  sub_irrdisp(lattice, symmetry, dynamic);          // creating irreducible displacement list

  end = clock();
  cpuTime= (end-start)/ (double) (CLOCKS_PER_SEC);
  start = clock();
  cout<<"Full FC list from Irreducible list created in  "<<cpuTime<<" seconds."<<endl;
  //=================================================================================================
...
//-------------------creating irreducible displacement list from full list-------------------------------------------------------------------
void sub_irrdisp(LATTICE *lattice, SYMMETRY *symmetry, DYNA *dynamic){
  int norder2, norder3, norder4;
  int atom1,atom2,atom3,atom4,atom11,atom22,atom33,atom44;


  
  // 2nd order------------------------------------------------------------------------
  atom1 = symmetry->rmap[dynamic->iorder2[4*0+0]];
  atom2 = symmetry->rmap[dynamic->iorder2[4*0+1]];
  norder2=1;
  for(int i=1;i<(dynamic->norder2)/2;i++){
    if(symmetry->rmap[dynamic->iorder2[4*i+1]]!=atom2){
      norder2++;
      atom2 = symmetry->rmap[dynamic->iorder2[4*i+1]];
    }
  }
  norder2 *= 9;
  
  symmetry->norder2 = norder2;
  symmetry->iorder2 = new int [4*norder2];
  
  atom1 = symmetry->rmap[dynamic->iorder2[4*0+0]];
  atom2 = symmetry->rmap[dynamic->iorder2[4*0+1]];
  norder2=0;
  for(int j=0;j<9;j++){
    symmetry->iorder2[4*norder2+0] = atom1;
    symmetry->iorder2[4*norder2+1] = atom2;
    symmetry->iorder2[4*norder2+2] = dynamic->iorder2[4*(0+j)+2];
    symmetry->iorder2[4*norder2+3] = dynamic->iorder2[4*(0+j)+3];
    norder2++;
  }
  for(int i=0;i<(dynamic->norder2)/2;i++){
    if(symmetry->rmap[dynamic->iorder2[4*i+1]]!=atom2){
      atom2 = symmetry->rmap[dynamic->iorder2[4*i+1]];
      for(int j=0;j<9;j++){
	symmetry->iorder2[4*norder2+0] = atom1;
	symmetry->iorder2[4*norder2+1] = atom2;
	symmetry->iorder2[4*norder2+2] = dynamic->iorder2[4*(i+j)+2];
	symmetry->iorder2[4*norder2+3] = dynamic->iorder2[4*(i+j)+3];
	norder2++;
      }
  ...
  for(int i=0;i<symmetry->norder2;i++){
    irorder2_out<<symmetry->iorder2[4*i+0]<<'\t'<<symmetry->iorder2[4*i+1]<<'\t';
    irorder2_out<<symmetry->iorder2[4*i+2]<<'\t'<<symmetry->iorder2[4*i+3]<<'\n';
  }
  //--------------------------------------------------------------------------------------------------------------------------------------
  ...
  //--------------3rd order---------------------------------------------------------------------------------------------------------------
  atom1 = symmetry->rmap[dynamic->iorder3[6*0+0]];
  atom2 = symmetry->rmap[dynamic->iorder3[6*0+1]];
  atom3 = symmetry->rmap[dynamic->iorder3[6*0+2]];
  atom22 = symmetry->rmap[dynamic->iorder3[6*0+1]];
  atom33 = symmetry->rmap[dynamic->iorder3[6*0+2]];
  norder3=1;
  for(int i=1;i<(dynamic->norder3)/2;i++){
    if(dynamic->iorder3[6*i+1]<dynamic->iorder3[6*i+2]){
      if((!((symmetry->rmap[dynamic->iorder3[6*i+1]]==atom2)&&(symmetry->rmap[dynamic->iorder3[6*i+2]]==atom3))) ){
	norder3++;
	atom2 = symmetry->rmap[dynamic->iorder3[6*i+1]];
	atom3 = symmetry->rmap[dynamic->iorder3[6*i+2]];
      }
    }
    if((dynamic->iorder3[6*i+1]==dynamic->iorder3[6*i+2])&&(dynamic->iorder3[6*i+2]==symmetry->rmap[dynamic->iorder3[6*i+2]])){
      if(!((dynamic->iorder3[6*i+1]==atom22))){
	norder3++;
	atom22 = symmetry->rmap[dynamic->iorder3[6*i+1]];
	atom33 = symmetry->rmap[dynamic->iorder3[6*i+2]];
      }
    }
  }
  norder3 *= 27;

  symmetry->norder3 = norder3;
  symmetry->iorder3 = new int [6*norder3];
  ...
  norder3=0;
  for(int j=0;j<27;j++){
    symmetry->iorder3[6*norder3+0] = atom1;
    symmetry->iorder3[6*norder3+1] = atom2;
    symmetry->iorder3[6*norder3+2] = atom3;
    symmetry->iorder3[6*norder3+3] = dynamic->iorder3[6*(0+j)+3];
    symmetry->iorder3[6*norder3+4] = dynamic->iorder3[6*(0+j)+4];
    symmetry->iorder3[6*norder3+5] = dynamic->iorder3[6*(0+j)+5];
    norder3++;
  }
  for(int i=0;i<(dynamic->norder3)/2;i++){
    if(dynamic->iorder3[6*i+1]<dynamic->iorder3[6*i+2]){
      if((!((symmetry->rmap[dynamic->iorder3[6*i+1]]==atom2)&&(symmetry->rmap[dynamic->iorder3[6*i+2]]==atom3))) ){
	atom2 = symmetry->rmap[dynamic->iorder3[6*i+1]];
	atom3 = symmetry->rmap[dynamic->iorder3[6*i+2]];
	for(int j=0;j<27;j++){
	  ...
	  symmetry->iorder3[6*norder3+1] = atom2;
	  symmetry->iorder3[6*norder3+2] = atom3;
	  symmetry->iorder3[6*norder3+3] = dynamic->iorder3[6*(i+j)+3];
	  symmetry->iorder3[6*norder3+4] = dynamic->iorder3[6*(i+j)+4];
	  symmetry->iorder3[6*norder3+5] = dynamic->iorder3[6*(i+j)+5];
	  norder3++;
	}
      }
    }
	...
	for(int j=0;j<27;j++){
	  symmetry->iorder3[6*norder3+0] = atom1;
	  symmetry->iorder3[6*norder3+1] = atom22;
	  symmetry->iorder3[6*norder3+2] = atom33+1;
	  symmetry->iorder3[6*norder3+3] = dynamic->iorder3[6*(i+j)+3];
	  symmetry->iorder3[6*norder3+4] = dynamic->iorder3[6*(i+j)+4];
	  symmetry->iorder3[6*norder3+5] = dynamic->iorder3[6*(i+j)+5];
	  norder3++;
  ...
  for(int i=0;i<symmetry->norder3;i++){
    irorder3_out<<symmetry->iorder3[6*i+0]<<'\t'<<symmetry->iorder3[6*i+1]<<'\t'<<symmetry->iorder3[6*i+2]<<'\t';
    irorder3_out<<symmetry->iorder3[6*i+3]<<'\t'<<symmetry->iorder3[6*i+4]<<'\t'<<symmetry->iorder3[6*i+5]<<'\n';
  }
  //---------------------------------------------------------------------------------------------------------
...
//----------------checking symmetries in the real structure--------------------------------------------------------------------
void sub_realSymmetry(LATTICE *lattice, SYMMETRY *symmetry){
  int atom_types,counter;
  double dist1,dist2;
  double x,y,z,xp,yp,zp;
  double modx,mody,modz,modxp,modyp,modzp;

  //checking different groups of atoms...same group are symmetric
  atom_types = 0;
  dist1 = lattice->pos[3*0+0]*lattice->pos[3*0+0]  +  lattice->pos[3*0+1]*lattice->pos[3*0+1] +  lattice->pos[3*0+2]*lattice->pos[3*0+2];
  for(int i=1;i<lattice->natom*lattice->ncell;i++){
    dist2 = lattice->pos[3*i+0]*lattice->pos[3*i+0]  +  lattice->pos[3*i+1]*lattice->pos[3*i+1] +  lattice->pos[3*i+2]*lattice->pos[3*i+2];
    if(!(dist2-dist1 <= (1e-5))&&(dist2-dist1 >= -(1e-5))){
      dist1 = dist2;
      atom_types++;
    }
  }
  ...
  symmetry->rot = new double [9*lattice->natom*lattice->ncell];
  symmetry->rmap = new int [lattice->natom*lattice->ncell];
  symmetry->rmap[0] = 0;
  counter =0;
  dist1 = lattice->pos[3*0+0]*lattice->pos[3*0+0]  +  lattice->pos[3*0+1]*lattice->pos[3*0+1] +  lattice->pos[3*0+2]*lattice->pos[3*0+2];
  for(int i=1;i<lattice->natom*lattice->ncell;i++){
     ...
     if(!(dist2-dist1 <= (1e-5))&&(dist2-dist1 >= -(1e-5))){
      dist1 = dist2;
      symmetry->rmap[i] = i;
      symmetry->rot[9*i+0] = 1; symmetry->rot[9*i+4] = 1; symmetry->rot[9*i+8] = 1;
      symmetry->rot[9*i+1] = 0; symmetry->rot[9*i+2] = 0; symmetry->rot[9*i+3] = 0;
      symmetry->rot[9*i+5] = 0; symmetry->rot[9*i+6] = 0; symmetry->rot[9*i+7] = 0;
      counter =i;
    }else{
      for(int j=0;j<9;j++){
	symmetry->rot[9*i+j]=0;
      }
      symmetry->rmap[i] = counter;
      x = lattice->pos[3*counter+0];
      y = lattice->pos[3*counter+1];
      z = lattice->pos[3*counter+2];
      xp = lattice->pos[3*i+0];
      yp = lattice->pos[3*i+1];
      zp = lattice->pos[3*i+2];
      modx = (x>0)?(x):(-x);
      mody = (y>0)?(y):(-y);
      modz = (z>0)?(z):(-z);
      modxp = (xp>0)?(xp):(-xp);
      modyp = (yp>0)?(yp):(-yp);
      modzp = (zp>0)?(zp):(-zp);
      if((modxp-modx<=1e-6)&&(modxp-modx>=-1e-6)){                // xp ==x
	if((modyp-mody<=1e-6)&&(modyp-mody>=-1e-6)){                  // xp==x, yp==y,zp==z
	  if((xp-x<=1e-6)&&(xp-x>=-1e-6)) symmetry->rot[9*i+0] =1;
	  else symmetry->rot[9*i+0] =-1;
	  if((yp-y<=1e-6)&&(yp-y>=-1e-6)) symmetry->rot[9*i+4] =1;
	  else symmetry->rot[9*i+4] =-1;
	  if((zp-z<=1e-6)&&(zp-z>=-1e-6)) symmetry->rot[9*i+8] =1;
	  else symmetry->rot[9*i+8] =-1;
	}else{								//xp==x,yp==z,zp==y
	  if((xp-x<=1e-6)&&(xp-x>=-1e-6)) symmetry->rot[9*i+0] =1;
	  else symmetry->rot[9*i+0] =-1;
	  if((yp-z<=1e-6)&&(yp-z>=-1e-6)) symmetry->rot[9*i+5] =1;
	  else symmetry->rot[9*i+5] =-1;
	  if((zp-y<=1e-6)&&(zp-y>=-1e-6)) symmetry->rot[9*i+7] =1;
	  else symmetry->rot[9*i+7] =-1;
	}
      }else{
	if((modxp-mody<=1e-6)&&(modxp-mody>=-1e-6)){                // xp==y
	  if((modyp-modx<=1e-6)&&(modyp-modx>=-1e-6)){          // xp==y, yp==x, zp ==z
	    if((xp-y<=1e-6)&&(xp-y>=-1e-6)) symmetry->rot[9*i+1] =1;
	    else symmetry->rot[9*i+1] =-1;
	    if((yp-x<=1e-6)&&(yp-x>=-1e-6)) symmetry->rot[9*i+3] =1;
	    else symmetry->rot[9*i+3] =-1;
	    if((zp-z<=1e-6)&&(zp-z>=-1e-6)) symmetry->rot[9*i+8] =1;
	    else symmetry->rot[9*i+8] =-1;
	  }else{						// xp==y, yp==z,zp==x
	    if((xp-y<=1e-6)&&(xp-y>=-1e-6)) symmetry->rot[9*i+1] =1;
	    ...
	    if((zp-x<=1e-6)&&(zp-x>=-1e-6)) symmetry->rot[9*i+6] =1;
	    else symmetry->rot[9*i+6] =-1;
	  }
	}else{                                             // xp==z
	  if((modyp-modx<=1e-6)&&(modyp-modx>=-1e-6)){   //xp ==z, yp==x, zp==y
	    if((xp-z<=1e-6)&&(xp-z>=-1e-6)) symmetry->rot[9*i+2] =1;
	    else symmetry->rot[9*i+2] =-1;
	    if((yp-x<=1e-6)&&(yp-x>=-1e-6)) symmetry->rot[9*i+3] =1;
	    else symmetry->rot[9*i+3] =-1;
	    if((zp-y<=1e-6)&&(zp-y>=-1e-6)) symmetry->rot[9*i+7] =1;
	    else symmetry->rot[9*i+7] =-1;
  ...
  for(int i=0;i<lattice->natom*lattice->ncell;i++){
    data_out<<lattice->pos[3*i+0]<<"\t"<<lattice->pos[3*i+1]<<"\t"<<lattice->pos[3*i+2]<<"\n";
    data_out<<lattice->pos[3*symmetry->rmap[i]+0]<<"\t"<<lattice->pos[3*symmetry->rmap[i]+1]<<"\t"<<lattice->pos[3*symmetry->rmap[i]+2]<<"\n";
    data_out<<symmetry->rot[9*i+0]<<'\t'<<symmetry->rot[9*i+1]<<'\t'<<symmetry->rot[9*i+2]<<'\n';
    data_out<<symmetry->rot[9*i+3]<<'\t'<<symmetry->rot[9*i+4]<<'\t'<<symmetry->rot[9*i+5]<<'\n';
    data_out<<symmetry->rot[9*i+6]<<'\t'<<symmetry->rot[9*i+7]<<'\t'<<symmetry->rot[9*i+8]<<'\n';
    data_out<<" "<<endl;
    data_out<<" "<<endl;
  }
*/
...
//----------creating symmetries for (0.25,0.25,0.25) atom
  symmetry->rot2 = new double [9];
  symmetry->rot2[0] = -1; symmetry->rot2[1] = 0; symmetry->rot2[2] = 0;
  symmetry->rot2[3] = 0; symmetry->rot2[4] = -1; symmetry->rot2[5] = 0;
  symmetry->rot2[6] = 0; symmetry->rot2[7] = 0; symmetry->rot2[8] = -1;

  symmetry->r2map = new int [lattice->natom*lattice->ncell];
  for(int i=0;i<lattice->natom*lattice->ncell;i++){
    symmetry->r2map[i] = -1;
  }
  for(int i=0;i<lattice->natom*lattice->ncell;i++){
    xp = symmetry->rot2[0]*(lattice->pos[3*i+0]-0.25)  +  symmetry->rot2[1]*(lattice->pos[3*i+1]-0.25)  +  symmetry->rot2[2]*(lattice->pos[3*i+2]-0.25);
    yp = symmetry->rot2[3]*(lattice->pos[3*i+0]-0.25)  +  symmetry->rot2[4]*(lattice->pos[3*i+1]-0.25)  +  symmetry->rot2[5]*(lattice->pos[3*i+2]-0.25);
    zp = symmetry->rot2[6]*(lattice->pos[3*i+0]-0.25)  +  symmetry->rot2[7]*(lattice->pos[3*i+1]-0.25)  +  symmetry->rot2[8]*(lattice->pos[3*i+2]-0.25);
    dist1 = xp*xp + yp*yp + zp*zp;
    for(int j=0;j<lattice->natom*lattice->ncell;j++){
      x = lattice->pos[3*j+0];
      y = lattice->pos[3*j+1];
      z = lattice->pos[3*j+2];
      dist2 = x*x + y*y + z*z;
      if((dist1-dist2<1e-4)&&(dist1-dist2>-1e-4)){
	if((xp-x<1e-4)&&(xp-x>-1e-4)){
	  if((yp-y<1e-4)&&(yp-y>-1e-4)){
	    if((zp-z<1e-4)&&(zp-z>-1e-4)){
	      symmetry->r2map[i] = j;
	    }
	  }
	}
  ...
  /*
  for(int i=0;i<lattice->natom*lattice->ncell;i++){
    data_out<<i<<'\t'<<lattice->pos[3*i+0]<<'\t'<<lattice->pos[3*i+1]<<'\t'<<lattice->pos[3*i+2]<<'\t';
    data_out<<lattice->pos[3*i+0]*lattice->pos[3*i+0] + lattice->pos[3*i+1]*lattice->pos[3*i+1] + lattice->pos[3*i+2]*lattice->pos[3*i+2]<<'\t'<<'\t';
    data_out<<lattice->pos[3*i+0]-0.25<<'\t'<<lattice->pos[3*i+1]-0.25<<'\t'<<lattice->pos[3*i+2]-0.25<<'\t'<<'\t';
    data_out<<(lattice->pos[3*i+0]-0.25)*(lattice->pos[3*i+0]-0.25) + (lattice->pos[3*i+1]-0.25)*(lattice->pos[3*i+1]-0.25)  + (lattice->pos[3*i+2]-0.25)*(lattice->pos[3*i+2]-0.25)<<'\t';
    data_out<<symmetry->r2map[i]<<endl;
  }
*/
  //-------------------------------------------------------------------------------------
