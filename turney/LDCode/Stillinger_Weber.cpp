...
void POT_SW2::ForceConstants(int b0, POT_DER_X *F) {
  ...
  for(n1=0;n1<n_mat;n1++) {
    //GET CUTOFF
    mat_pos[1] = n1;
    cutoff_ = a[(n0<=n1) ? n_mat*n0-((n0+1)*n0)/2+n1 : n_mat*n1-((n1+1)*n1)/2+n0];
...
double POT_SW3::Energy(double **X) {
	...
	j = mat_pos[1];
	k = mat_pos[2];
	dim = (j<=k) ? n_mat*j-((j+1)*j)/2+k : n_mat*k-((k+1)*k)/2+j;
  eps = epsilon[i][dim];
	lam = lambda[i][dim];
	j = (i<=j) ? n_mat*i-((i+1)*i)/2+j : n_mat*j-((j+1)*j)/2+i;
	k = (i<=k) ? n_mat*i-((i+1)*i)/2+k : n_mat*k-((k+1)*k)/2+i;
...
/**mat[n0] AT VERTEX**/
    ...
    for(n2=n1;n2<n_mat;n2++) {
      ...
      cutoff_ = a[(n0<=n2) ? n_mat*n0-((n0+1)*n0)/2+n2 : n_mat*n2-((n2+1)*n2)/2+n0];
      n_neig2 = new int;
      N_list2 = new N_LIST*;
      if(n1==n2) {
        n_neig2[0] = n_neig1;
...
/**mat[n0] ON END**/
    for(n2=0;n2<n_mat;n2++) {
      //BUILD NEIGHBOR LISTS
      cutoff_ = a[(n1<=n2) ? n_mat*n1-((n1+1)*n1)/2+n2 : n_mat*n2-((n2+1)*n2)/2+n1];
