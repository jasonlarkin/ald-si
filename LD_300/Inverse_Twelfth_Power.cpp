/*						         Inverse_Twelfth_Power.cpp		         				*/
/*							              12/02/2008		              					*/
/*********************************************************************
*    File that contains the POT_INV12 class function definitions. 	 *
*  POT_INV12 is a derivative of the POTENTIAL class.		          	 *
*********************************************************************/
...
/*POT_INV12 CONSTRUCTOR: STORES POTENTIAL AND POTENTIAL PARAMETER NAMES*/
POT_INV12::POT_INV12(int n_mat_, int *mat_) : POTENTIAL(n_mat_, mat_) {
  Pot_Symb = new char[6];
  strcpy(Pot_Symb, "INV12");
  mat_pos = new int[2];
...
/*POT_INV12 ReadInput: READS AND STORES POTENTIAL PARAMETER VALUES*/
string POT_INV12::ReadInput(ifstream &Input) {
  string str;
  while(!Input.eof()) {
    str = Read_Next(Input);
...
/*POT_INV12 FUNCTION GetScale: INITIALIZES INITIALIZES THE LENGTH AND ENERGY SCALES*/
void POT_INV12::GetScale(double &e, double &l) {
  e = 0.0;
  l = 0.0;
	for(int i=0;i<n_expect;i++) {
	  e += epsilon[i];
...
/*POT_INV12 FUNCTION Initialize: INITIALIZES THE POTENTIAL PARAMETERS*/
void POT_INV12::Initialize() {
	for(int i=0;i<n_expect;i++) {
...
/*POT_INV12 FUNCTION Copy: RETURNS A POINTER TO A NEW POTENTIAL WITH A DUPLICATE OF THE DATA*/
POTENTIAL *POT_INV12::Copy() {
  POT_INV12 *P_new = new POT_INV12(n_mat, mat);
  for(int i=0;i<n_expect;i++) {
...
/*POT_INV12 FUNCTION Energy: COMPUTES THE POTENTIAL ENERGY*/
double POT_INV12::Energy(double **X) {
  int i, j;
	double r_ij2 = 0.0;
	double r_ij6;
	//
	...
	double phi = epsilon[j]*r_ij6*r_ij6 ;

	return(phi);
}


/*POT_INV12::ForceConsants: COMPUTES THE HARMONIC FORCE CONSTANTS*/
void POT_INV12::ForceConstants(int b0, POT_DER_X *F) {
...
/*POT_INV12 DESTRUCTOR: DEALLOCATES MEMORY*/
POT_INV12::~POT_INV12() {
