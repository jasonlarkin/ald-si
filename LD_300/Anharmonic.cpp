...
void ANHARMONIC::Compute(int iter) {
  ...
  int **wv_N;                 //Wave vector counters for outer, middle, and inner loops
	...
	double per_ps, DV, PV;		  //Dimensionalizing factor (THz) or (1/s), Delta value
...
/*    //Check if we are continuing from a partial calculation
    ...
    if(ah_f_C[0]<1.0e-8) {  //Skip bulk translation
      ...
      Progress(mntr, loop, Sym_F, begin);
      continue;
    }
    ...
    wv[1][2] = double(wv_N[1][2])/double(UC[2]);
      ...
      for(nu[1]=0;nu[1]<UC_DOF;nu[1]++) {
        ...
        for(nu[2]=0;nu[2]<UC_DOF;nu[2]++) {
          ...
          T = T/ah_f[1]/ah_f[2];
          e = ah_lw[0]+ah_lw[1]+ah_lw[2];
          PV_DV(ah_f, n1, n2, e, PV, DV);
          Gamma += T*DV;
#if defined FREQ_SHIFT
          Delta3 += T*PV;
#endif

          e = ah_lw_C[0]+ah_lw_C[1]+ah_lw_C[2];
          PV_DV(ah_f_C, n1_C-0.5, n2_C-0.5, e, PV, DV);
          Gamma_C += T_C*DV;
#if defined FREQ_SHIFT
          Delta3_C += T_C*PV;
#endif
    ...
    T = freq[0]*1.0e-12/Parameter->time;
    Write(loop, T, Delta3, Delta3_C, Delta4, Delta4_C, Gamma, Gamma_C, S_W_Out, S_W_Out_C);
    Progress(mntr, loop+nPE, Sym_F, begin);
...
void ANHARMONIC::Progress(int &mntr, int loop, int Sym_F, int begin) {
  if(PE!=0) {return;}
  if(Sym_F*mntr<=100*loop) {
    Log <<"Anharmonic calculation is "<<int(double(loop)/Sym_F*100)%100
      <<"% complete after "<<double(time(0)-begin)/60.0<<" minutes.";
    Log <<"  "<<loop<<" of "<<Sym_F<<" waves calculated."<<endl;
    cout <<int(double(loop)/Sym_F*100)%100<<"% done after "
